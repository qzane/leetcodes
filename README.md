...
Sliding window, and use two [Monotonic Queue](https://leetcode.com/problems/shortest-subarray-with-sum-at-least-k/discuss/204290/Monotonic-Queue-Summary) to record the maxs and mins inside the window.

# 1439. Find the Kth Smallest Sum of a Matrix With Sorted Rows
Since k is really small (<200), we can begin with the smallest sum (the first item in each rows), and use a priority queue (heap) to record all candidate sums. The candidates are generated by push back the pointer in any rows, which means there are at most m candidates in each step. We also need a hashset to prevent any selection being added to the queue for multiple times.
